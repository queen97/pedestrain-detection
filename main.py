...
cam = cv2.VideoCapture('../video/pedestrians.avi')  # (1)
...
while True:
    ...
    rects_ms = non_max_suppression(rects, probs=None, overlapThresh=0.4)
    ...
    for (xA, yA, xB, yB) in rects_ms:
        cv2.rectangle(frame_rgb, (xA+20, yA+20), (xB-10, yB-10), (0, 255, 0), 2)
    for cog in cogs:
...
# https://docs.opencv.org/3.4.1/d9/dab/tutorial_homography.html
    ...
    # Time elapsed
    seconds = end - start
    cv2.putText(frame_rgb, "FPS:{}".format(np.round(8/seconds, 0)), (20, 255),
